...
/** Rendu sous SettingsProvider (la table est i18n depuis POLISH-3) — langue FR par défaut. */
function render(props: Parameters<typeof CountryTable>[0]): string {
  return renderToStaticMarkup(
    createElement(SettingsProvider, null, createElement(CountryTable, props)),
  );
}
...
describe("CountryTable — vue réduite par défaut", () => {
  it("montre pays + tendance, pas les 5 colonnes de chiffres", () => {
    const html = render({ worldCountries: world, history });
...
describe("CountryTable — densité Expert (defaultDetailed)", () => {
  it("affiche les 5 colonnes d'office", () => {
    const html = render({ worldCountries: world, history, defaultDetailed: true });
...
describe("CountryTable — ta ligne en avant (fusion « Ta position »)", () => {
  it("le pays joué passe en tête et porte la pastille « toi »", () => {
    const html = render({ worldCountries: world, history, playAs: "usa" });
